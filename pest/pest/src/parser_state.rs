...
/// ```
pub fn state<R: RuleType, F>(input: Rc<String>, f: F) -> Result<pairs::Pairs<R>, Error<R>>
where
...
impl<R: RuleType> ParserState<R> {
    ...
    pub fn new(input: Rc<String>) -> Box<Self> {
        Box::new(ParserState {
    ...
    pub fn stack_peek(self: Box<Self>) -> ParseResult<Box<Self>> {
        let string = self.stack.peek().expect("peek was called on empty stack");
        let string = string.as_str().to_owned();
    ...
    pub fn stack_pop(mut self: Box<Self>) -> ParseResult<Box<Self>> {
        let o = self.stack.pop().expect("pop was called on empty stack");
        let string = o.as_str();
        self.match_string(string)
    }
